    ...
    */
    ...
    integer commandChannel = 1967;  // Command channel in chat (release year of album "The Doors")
...
//  integer LM_ME_CHANGED = 130;    // Position/rotation change report to compiler
    ...
    integer linkNick(string n) {
        if (abbrP(n, "tu")) {
            return flGetCompLink("Turntable");
        } else if (abbrP(n, "do")) {
            return flGetCompLink("Door 1");
        }
        return (integer) n;
    }
    ...
    genRot(string compname, vector axis, string angle) {
        ...
        tawk("  Confirming new rot " + efr(flGetCompRot(linkno)));
        scriptSuspend = TRUE;               // Suspend for completion if in script
    }

    //  openClose  --  Toggle door open/closed

    integer OPENMODE = 0;           // KLUDGE DOOR OPEN/CLOSE MODE

    openClose() {
        if (OPENMODE == 0) {
            //  Begin open sequence
            llPlaySound("e5e01091-9c1f-4f8c-8486-46d560ff664f", 1);
            //  Rotate door handle
            flSetCompOmega(2, <0, 0, 1>, 1, 1, 45 * DEG_TO_RAD);
            OPENMODE = 1;
        } else if (OPENMODE == 4) {
            //  Begin close sequence
            OPENMODE = 5;
            //  Start door panel closing
            flSetCompOmega(9, <0, 0, 1>, 0.5, 1, 75 * DEG_TO_RAD);
            llPlaySound("Creaking door", 1);
        }
    }
    ...
    integer processCommand(key id, string message, integer fromScript) {
        ...
        //  Door open/close             Open / close door

        } else if (abbrP(command, "do")) {
            openClose();
        ...
        //  RH ang                      Rotate Handle

        } else if (abbrP(command, "rh")) {
            /*  Have to specify handle rotation in parent (Panel) coords,
                rotated <0, 90, 0>, with rotation about Z of the handle shaft.  */
            flSetCompRot(flGetCompLink("Door handle shaft"),
                llEuler2Rot(<0, 90, 0> * DEG_TO_RAD) *
                    llEuler2Rot(<(float) sparam, 0, 0> * DEG_TO_RAD));
            scriptSuspend = TRUE;
        ...
        //  RP ang                      Rotate Panel of door

        } else if (abbrP(command, "rp")) {
            genRot("Hinge top", <0, 0, 1>, sparam);
        ...
        //  Trans linkno <x, y, z>      Translate linkno by <x, y, z>
            ...
            integer linkno = linkNick(sparam);
            vector where = flGetCompPos(linkno);
            flSetCompPos(linkno, where + (vector) llList2String(args, 2));
            scriptSuspend = TRUE;
...
//tawk("Mechanism link message sender " + (string) sender + "  num " + (string) num + "  str " + str + "  id " + (string) id);
            ...
            //  LM_ME_OMEGALIMIT (129): Omega rotation reached preset limit

            } else if (num == LM_ME_OMEGALIMIT) {
//tawk("Omega rotation of link " + str + " (" + flGetCompName((integer) str) + ") reached limit.");
                //  Little state machine to sequence door opening and closing
                if (OPENMODE == 1) {
                    ...
                    //  Handle restore to neutral complete
                    OPENMODE = 3;
                } else if (OPENMODE == 3) {
                    //  Door open complete.  Idle until close issued
                    OPENMODE = 4;
                                scriptMotion = FALSE;
                                scriptResume();
                } else if (OPENMODE == 5) {
                    //  Door close complete.
                    llPlaySound("88d13f1f-85a8-49da-99f7-6fa2781b2229", 1);
                    OPENMODE = 0;
                    scriptMotion = FALSE;
                    scriptResume();
                }
            ...
            //  LM_ME_COMPLETE (131): Translate/rotate complete
                ...
                if ((op == LM_ME_ROTSTEP) || (op == LM_ME_TRANSTEP)) {
                    return;
                }
                if (trace) {
                    string sop = "Rotation ";
