    ...
    */
    ...
    integer commandChannel = 1931;  // Command channel in chat (year of first coin-operated pinball machine)
    ...
    float tiltMax = 8;              // Tilt maximum angle, degrees
    ...
    list marbles = [
        1, <0.58989, 1.08426, 0.80798>,
        2, <-0.58487, 0.00000, 0.80798>,
        3, <0.00000, -1.13757, 0.80798>,
        4, <0.94476, 0.50000, 0.80798>,
        5, <0.93273, -0.75475, 0.80798>,
        6, <-0.92008, 0.65205, 0.80798>
    ];
...
//  integer LM_ME_CHANGED = 130;    // Position/rotation change report to compiler
    ...
    //  sendSettings  --  Send settings to auxiliary scripts

    sendSettings(key id) {
        llMessageLinked(LINK_ALL_OTHERS, LM_AU_SETTINGS,
            llList2CSV([ angleScale, trace, volume ]), id);
    }
    ...
    /*  slerp  --  Spherical linear interpolation between two rotations
                   with a parameter between 0 and 1, per:
                      Shoemake, Ken, ""Animating Rotation with Quaternion
                      Curves". SIGGRAPH, 1985.
                   See:
...
/*
    rotation slerp(rotation a, rotation b, float t) {
        ...
        return llAxisAngle2Rot(llRot2Axis(diff), t * llRot2Angle(diff)) * a;
    }
*/
    ...
    integer processCommand(key id, string message) {
        ...
        //  Tilt
            ...
            if (!tilting) {
                tiltLink = flGetCompLink("Table base");
                tiltRotOrig = flGetCompRot(tiltLink);  // Save original table rotation
...
//tawk("mechRad " + (string) mechRad + "  " + (string) mechScale);
...
//tawk("MarbleChan " + (string) marbleChan);
